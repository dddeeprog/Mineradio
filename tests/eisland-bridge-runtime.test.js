...
test('wires_web_player_to_restricted_eisland_runtime', () => {
  ...
  const playerAdapter = html.match(/function getEislandBridgePlayer\(\) \{[\s\S]*?\n\}\nfunction publishEislandBridgePlayerEvent/)?.[0] || '';
  for (const field of ['playQueue', 'currentIdx', 'lyricsLines', 'lyricsLifecycle', 'bridgeTransitionFailed', 'bridgeTransitionPending']) {
    assert.match(playerAdapter, new RegExp(field));
  }
  assert.doesNotMatch(playerAdapter, /currentDesktopSongMeta|currentDesktopLyricSnapshot/);
  assert.match(playerAdapter, /if \(transition\.failed\) \{[\s\S]*?attemptAudioPlay\(\{ manual: true \}\)[\s\S]*?playQueueAt\(currentIdx, \{ manual: true \}\)\.then\(/);
  ...
  assert.match(html, /beginEislandBridgeTrackTransition\(token\);/);
  assert.match(html, /settleEislandBridgeTrackTransition\(token, true\);/);
  assert.match(html, /settleEislandBridgeTrackTransition\(token, false\);/);
  assert.match(html, /\['loadedmetadata', 'timeupdate', 'play', 'pause', 'ended', 'seeked', 'ratechange'\]/);
  assert.equal((html.match(/audio\._mineradioBridgeTrackToken = token;/g) || []).length, 2);
  ...
  assert.equal((trackControls.match(/\.then\(function\(result\)\{ return !!\(result && result\.ok\); \}\)/g) || []).length, 2);
...
test('checks_eisland_bridge_runtime_and_lifecycle_syntax', () => {
  ...
  assert.match(packageJson.scripts.precheck, /node --check public\/eisland-bridge-runtime\.js/);
  assert.match(packageJson.scripts.precheck, /node --check desktop\/eisland-bridge-lifecycle\.js/);
});
