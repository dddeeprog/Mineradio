(function(root) {
  ...
  function createUpdatePreviewUi() {
var UPDATE_NOTICE_SUPPRESSED_FOR_MAINTENANCE = true;
...
function suppressUpdatePreviewForMaintenance() {
  ...
  if (!entry) return;
  entry.classList.remove('available', 'downloading', 'ready');
  if (window.gsap) {
...
function initUpdatePreview() {
  if (UPDATE_NOTICE_SUPPRESSED_FOR_MAINTENANCE) {
    suppressUpdatePreviewForMaintenance();
    return;
  }
...
function pulseUpdateReady() {
  ...
  if (btn) {
    ...
    return {
      ...
      suppressUpdatePreviewForMaintenance: suppressUpdatePreviewForMaintenance,
      ...
      getState: function() { return updatePreviewState; },
    };
  }

  var api = { init: createUpdatePreviewUi };
  if (typeof window !== 'undefined') window.MineradioUpdatePreviewUi = api;
  else root.MineradioUpdatePreviewUi = api;
