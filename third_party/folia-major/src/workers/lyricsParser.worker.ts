 ...
 *
 * Message API:
 * Request: { type: 'parse', format: 'lrc' | 'enhanced-lrc' | 'yrc' | 'qrc' | 'vtt' | 'ttml', content: string, translation?: string, requestId?: string }
...
const normalizeWorkerFormat = (format: string): LyricParseFormat => {
    if (format === 'yrc' || format === 'qrc' || format === 'enhanced-lrc' || format === 'vtt' || format === 'ttml') {
        return format;
    }
