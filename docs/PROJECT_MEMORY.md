...
### 2026-07-28 - XxHuberrr 功能融合范围

- 用户认可/要求保留：以 `codex/unified-player` 为底座，XxHuberrr 项目第 1 类只接入已点名的网易云写操作、五平台统一搜索、平台能力声明、账号隔离缓存、跨平台收听统计和多平台登录中心；第 2–7 类照常接入；第 8–9 类与当前实现逐项比较，只融合更好的部分。
- 涉及文件：`docs/superpowers/specs/2026-07-28-xxhuberrr-capability-migration-design.md`，后续按规格涉及平台、播放、Cuefield、视觉、完整桌面、Wallpaper Engine、性能、安装和工程模块。
- 关键参数/实现：酷狗、汽水和 Spotify 首期只提供搜索及登录中心所需能力，不接完整播放/写入；汽水音频解密器不迁移；前端必须按能力声明显示操作；保留当前集中式 IPC/服务端安全、更新摘要校验、发布验证、Folia、本地库和 `English-worse/Mineradio` 发布归属。
- 禁止回退或改坏的点：不要整仓覆盖当前分支；不要因为迁移第 2–7 类而暗中恢复被排除的平台播放路径；不要用远端安全、更新或测试实现替换当前更强的基础设施；账号缓存必须按平台和账号指纹隔离。
