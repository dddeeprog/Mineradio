...
| 旧视觉预设 | `public/folia-native/config.js` | 视觉参考 | Task 8 已迁移：旧 `foliaInspiredPreset` 的 `classic`、`partita`、`monet` 值映射到现有原生模式；显式 `visualMode` 保持优先，显式 `false`、无效或缺失 preset 安全回退 `mineradio-3d`。 |
