...
/**
 * Adapted from XxHuberrr/Mineradio desktop/wallpaper-engine-runtime.js at
...
public static class MineradioWeWindowControl {
  ...
  static bool RemoveTaskbarTab(IntPtr hWnd) {
  ...
  static MineradioWeWindowResult RunDpiAware(string action, string sourceId, string expectedTitle, string expectedExecutable, string hostWindowId, string hostExecutable, string hostCornerRadius) {
    ...
    if (String.Equals(action, "park", StringComparison.OrdinalIgnoreCase)) {
      ...
      try {
        taskbarHidden = RemoveTaskbarTab(hWnd);
      } catch { }
      if (taskbarHidden) parkResult.taskbarHidden = true;
      return parkResult;
    }
...
class WallpaperEngineRuntime {
  ...
  _publicSession(session) {
    ...
    return {
      ...
      sourceWindowTaskbarHidden: !!(session.windowParking && session.windowParking.taskbarHidden),
  ...
  _powerShellEnv(extra = {}) {
    ...
    if (this.platform === 'win32') {
      ...
      const moduleKey = windowsPowerShellModules.toLowerCase();
      const existing = String(env.PSModulePath || '')
        .split(path.delimiter)
        .filter((entry) => entry && path.resolve(entry).toLowerCase() !== moduleKey);
      env.PSModulePath = [windowsPowerShellModules, ...existing].join(path.delimiter);
    }
    return env;
  }
  ...
  async _nativeWindowControl(action, details = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        const helperSource = Buffer.from(nativeWindowControlScript(), 'base64').toString('utf16le');
        const helperDigest = crypto.createHash('sha256').update(helperSource).digest('hex').slice(0, 20);
        const helperFile = path.join(this.nativeTempPath, `wallpaper-engine-window-control-${helperDigest}.ps1`);
        if (!fs.existsSync(helperFile)) {
          fs.writeFileSync(helperFile, `\uFEFF${helperSource}`, 'utf8');
        }
